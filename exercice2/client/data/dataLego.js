...
let DataLego = {};
 ...
 /**
     ...
     *
     * @param {string} collection - The collection parameter to be sent to the server.
     * @returns The response from the server.
     * 
     * DataLego.request permet de récupérer des données depuis le serveur.
     * Elle prend en paramètre une collection de Lego et renvoie les données contenues
     * dans la réponse du serveur (data).
     */
DataLego.request = async function(collection){
    // fetch permet d'envoyer une requête HTTP à l'URL spécifiée. 
    // L'URL est construite en concaténant HOST_URL à "/server/script.php?direction=" et la valeur de la variable dir. 
    // L'URL finale dépend de la valeur de HOST_URL et de dir.
    let answer = await fetch(HOST_URL + "/server/script.php?collection=" + collection);
